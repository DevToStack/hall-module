...
export async function createTempBooking(bookingData) {
    ...
    try {
        ...
        const [apartmentResult] = await connection.execute(
            `SELECT id, daily_rate, max_guests FROM apartments WHERE id = ?`,
        ...
        const [insertResult] = await connection.execute(
            ...
            [userId, bookingData.apartment_id, bookingData.start_date, bookingData.end_date, 'pending', expiresAt]
        );

        const bookingId = insertResult.insertId;

        // === 7. STORE ADDITIONAL BOOKING DETAILS ===
        await connection.execute(
            `INSERT INTO booking_details (
                booking_id, guests, total_amount, nights
            ) VALUES (?, ?, ?, ?)`,
            [bookingId, bookingData.guests, bookingData.total_amount, bookingData.nights]
        );
